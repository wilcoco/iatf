...
import { getActiveSuppliers, getActiveParts, type Supplier, type Part } from "@/lib/master-data";
...
export default function ReceivingInspectionPage() {
  ...
  const activeParts = useMemo(() => getActiveParts(), []);
  ...
  const handleSupplierChange = (supplierCode: string) => {
    const supplier = activeSuppliers.find((s) => s.code === supplierCode);
    setFormData({
      ...formData,
      supplierCode,
  ...
  // Handle part selection for standards
  const handleStandardPartChange = (partCode: string) => {
    const part = activeParts.find((p) => p.code === partCode);
    setStandardFormData({
      ...standardFormData,
      partCode,
  ...
  const handleSaveInspection = () => {
    if (!formData.supplierCode || !formData.partCode || !formData.inspector) {
  ...
  const handleSaveStandard = () => {
    if (!standardFormData.partCode || !standardFormData.partName || !standardFormData.inspectionItem) {
      alert("필수 항목을 모두 입력해주세요.");
      return;
    }
  ...
  const filteredStandards = standards.filter(
    (s) =>
      s.partCode.toLowerCase().includes(standardSearch.toLowerCase()) ||
      s.partName.toLowerCase().includes(standardSearch.toLowerCase()) ||
  ...
  const filteredNonconformities = nonconformities.filter(
    (nc) =>
      ...
      nc.supplierName.toLowerCase().includes(ncSearch.toLowerCase()) ||
      nc.partCode.toLowerCase().includes(ncSearch.toLowerCase())
  );
  ...
  const supplierStats = statusInspections.reduce((acc, insp) => {
    const supplier = insp.supplierName;
    if (!acc[supplier]) {
      ...
      <Card>
        <CardContent className="pt-6">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {/* Tab 1: 수입검사 등록 (Receiving Inspection Entry) */}
              ...
              <div className="space-y-6">
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {/* Part Info */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold border-b pb-2">품목 정보</h3>
                      <div className="grid gap-4 md:grid-cols-4">
                        <div className="space-y-2">
                          <Label>품목코드 *</Label>
                          <Select
                        ...
                        <TableBody>
                          {filteredInspections.map((insp) => (
                            <TableRow key={insp.id}>
                              ...
                              <TableCell className="font-mono">{insp.partCode}</TableCell>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                        ...
                        <CardContent className="pt-6 space-y-4">
                          ...
                          <div className="grid gap-4 md:grid-cols-4">
                            <div className="space-y-2">
                              <Label>품목코드 *</Label>
                              <Select
                                value={standardFormData.partCode}
                                onValueChange={handleStandardPartChange}
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="품목 선택" />
                                </SelectTrigger>
                            ...
                            <div className="space-y-2">
                              <Label>품목명</Label>
                              <Input
                                value={standardFormData.partName}
                                disabled
                                className="bg-muted"
                              />
                            </div>
                    ...
                    <div className="relative max-w-md">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                      <Input
                        placeholder="품목코드, 품목명, 검사항목으로 검색..."
                      ...
                      <TableBody>
                        {filteredStandards.map((std) => (
                          <TableRow key={std.id} className={!std.isActive ? "opacity-50" : ""}>
                            <TableCell className="font-mono">{std.partCode}</TableCell>
                            <TableCell>{std.partName}</TableCell>
                        ...
                        <TableBody>
                          {filteredNonconformities.map((nc) => (
                            <TableRow key={nc.id}>
                              ...
                              <TableCell>{nc.supplierName}</TableCell>
                              <TableCell className="font-mono">{nc.partCode}</TableCell>
                              <TableCell>{nc.partName}</TableCell>
                              <TableCell>
